...
float DoSpecular(float3 V, float3 N, float3 L, float specularPower)
{
    ...
    float originalSpecular = pow(NdotH, specularPower);
    float toonSpecular = step(0.5, originalSpecular);
    return toonSpecular;
}

float DoRim(float3 V, float3 N, float3 L, float rimThreshold, float rimPower)
{
    float VdotN = dot(V, N);
    float NdotL = dot(N, L);
    float rim = 1.0 - VdotN;
    float rimValue = pow(rim, rimPower);
    float toonRim = step(rimThreshold, rimValue * NdotL);
    return toonRim;
}
...
LightResult DoDirectionalLight(DirectionalLight light, float3 V, float3 P, float3 N, float specularPower)
{
    ...
    result.Rim = light.color * light.intensity * DoRim(V, N, L, 0.5, 4.0);
...
float4 PSMain(PSInput input) : SV_TARGET
{
    ...
    for (uint i = 0; i < LightCB.numDirectionalLights; ++i)
    {
        ...
        totalResult.Specular += result.Specular * 0.5f;  // TODO specular light intensity
        totalResult.Ambient += result.Ambient;
        totalResult.Rim += result.Rim;
    }
    ...
    float4 color = (totalResult.Ambient + totalResult.Diffuse + totalResult.Rim) * objectColor + totalResult.Specular;

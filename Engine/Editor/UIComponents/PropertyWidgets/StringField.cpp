...
{
    if (!label || !buf || bufSize == 0)
    {
        DLOG(LogUIComponents, ELogLevel::Warning,
            "StringField::Draw: expected non-null label and buf with bufSize>0 (label={}, buf={}, bufSize={})",
            static_cast<const void*>(label), static_cast<const void*>(buf), bufSize);
        return {};
    }
    ...
    {
        DLOG(LogUIComponents, ELogLevel::Warning,
            "StringField::Draw: no EditorTheme (expected g_editor or UIComponents test theme override)");
        return {};
    }

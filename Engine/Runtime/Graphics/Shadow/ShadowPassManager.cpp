...
void ShadowPassManager::Initialize(Device& device, const ShadowAtlasSettings& atlasSettings)
{
    Shutdown();
    m_atlasSettings = atlasSettings;
    m_skipRenderIssuesLogged = false;
    m_directionalAtlas.Initialize(device, m_atlasSettings.atlasSize, "ShadowAtlas Directional");
    m_spotAtlas.Initialize(device, m_atlasSettings.atlasSize, "ShadowAtlas Spot");
    m_pointCubes.Initialize(device, m_atlasSettings.pointFaceSize, m_atlasSettings.pointCubeCount, "ShadowCubeArray Point");
    try
    {
    ...
    else
    {
        DLOG(LogShadow, ELogLevel::Log,
            ...
            m_atlasSettings.atlasSize, m_atlasSettings.directionalTileSize, m_atlasSettings.spotTileSize,
            m_atlasSettings.pointFaceSize, m_atlasSettings.pointCubeCount);
    }
}
...
void ShadowPassManager::Render(std::shared_ptr<DXGraphicsContext> ctx, DWorld& world)
{
    ...
    m_directionalAllocator.Reset(m_atlasSettings.atlasSize, m_atlasSettings.atlasSize, m_atlasSettings.directionalTileSize);
    m_spotAllocator.Reset(m_atlasSettings.atlasSize, m_atlasSettings.atlasSize, m_atlasSettings.spotTileSize);
    m_pointAllocator.Reset(m_atlasSettings.pointCubeCount);
            ...
            if (cubeIndex < 0)
            {
                if (m_warnedPoint.insert(view.lightIndex).second)
                {
                    DLOG(LogShadow, ELogLevel::Warning,
                        ...
                        view.lightIndex, m_atlasSettings.pointCubeCount);
                }
                continue;
            }
    ...
    if (!pointJobs.empty())
    {
        ...
        viewport.Width = static_cast<float>(m_atlasSettings.pointFaceSize);
        viewport.Height = static_cast<float>(m_atlasSettings.pointFaceSize);
        viewport.MinDepth = 0.0f;
        viewport.MaxDepth = 1.0f;

        const D3D12_RECT scissor = { 0, 0,
            static_cast<LONG>(m_atlasSettings.pointFaceSize), static_cast<LONG>(m_atlasSettings.pointFaceSize) };
    ...
    for (const DirJob& job : dirJobs)
    {
        ...
        ShadowMapAllocator::FillAtlasUVRect(m_atlasSettings.atlasSize, m_atlasSettings.atlasSize, job.region, alloc);

...
#include "Runtime/Graphics/DirectX/GenerateMipsPSO.h"
#include "Runtime/Graphics/DirectX/PanoToCubemapPSO.h"
    ...
    Microsoft::WRL::ComPtr<ID3D12Resource> srcRes)
{
    ...
    TransitionBarrier(dstRes, m_d3d12CommandListType == D3D12_COMMAND_LIST_TYPE_COPY ? D3D12_RESOURCE_STATE_COMMON : D3D12_RESOURCE_STATE_COPY_DEST);
    TransitionBarrier(srcRes, m_d3d12CommandListType == D3D12_COMMAND_LIST_TYPE_COPY ? D3D12_RESOURCE_STATE_COMMON : D3D12_RESOURCE_STATE_COPY_SOURCE);
    ...
    const void* bufferData)
{
    ...
    std::shared_ptr<StructuredBuffer> structuredBuffer = m_Device.CreateStructuredBuffer(d3d12Resource, numElements, elementSize);
...
void CommandList::GenerateMips(const std::shared_ptr<DirectX12Texture>& texture)
{
    ...
    if (!texture->CheckUAVSupport() || (resourceDesc.Flags & D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS) == 0) {
        ...
        uavDesc.Format = DirectX12Texture::GetUAVCompatableFormat(resourceDesc.Format);
    ...
    GenerateMips_UAV(uavTexture, DirectX12Texture::IsSRGBFormat(resourceDesc.Format));
...
void CommandList::GenerateMips_UAV(const std::shared_ptr<DirectX12Texture>& texture, bool isSRGB)
{
    ...
    srvDesc.Format = isSRGB ? DirectX12Texture::GetSRGBFormat(resourceDesc.Format) : resourceDesc.Format;
...
void CommandList::PanoToCubemap(const std::shared_ptr<DirectX12Texture>& cubemapTexture,
    const std::shared_ptr<DirectX12Texture>& panoTexture)
{
    ...
    D3D12_UNORDERED_ACCESS_VIEW_DESC uavDesc = {};
    uavDesc.Format = DirectX12Texture::GetUAVCompatableFormat(cubemapDesc.Format);
    ...
    for (uint32_t mipSlice = 0; mipSlice < cubemapDesc.MipLevels;) {
        ...
        Dispatch(DivideByMultiple(panoToCubemapCB.CubemapSize, 16),
            DivideByMultiple(panoToCubemapCB.CubemapSize, 16), 6);
...
void CommandList::ClearTexture(const std::shared_ptr<DirectX12Texture>& texture, const float clearColor[4])
{
    assert(texture);
...
void CommandList::ClearDepthStencilTexture(const std::shared_ptr<DirectX12Texture>& texture, D3D12_CLEAR_FLAGS clearFlags,
...
void CommandList::SetConstantBufferView(uint32_t rootParameterIndex, uint32_t descriptorOffset,
    const std::shared_ptr<ConstantBufferView>& cbv, D3D12_RESOURCE_STATES stateAfter)
{
    assert(cbv);

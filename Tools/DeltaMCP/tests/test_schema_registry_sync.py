...
def test_cpp_registry_matches_schema_operations():
    cpp_registry, _ = parse_cpp_registrations(CPP_SYSTEMS_DIR)
    schema_registry = load_schema_registry(SCHEMAS_DIR)
    errors = compare_operation_registries(cpp_registry, schema_registry)
    assert errors == [], "\n".join(errors)


def test_schema_params_match_cpp_handlers():
    _, operations = parse_cpp_registrations(CPP_SYSTEMS_DIR)
    schema_params = load_schema_params(SCHEMAS_DIR)
    errors = compare_operation_params(operations, schema_params)
    assert errors == [], "\n".join(errors)
...
def test_local_meta_queries_subset_of_schema_meta_queries():
    meta_queries = schema_registry = load_schema_registry(SCHEMAS_DIR)["meta"]["queries"]
